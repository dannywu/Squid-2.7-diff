...
//---add by wuhongxing 20120720
static const char *fetchuri_dir(const char *uri);

static const char *fetchuri_dir(const char *uri)
{
  char *ptr = NULL;
  static char uri_dir[MAX_URL];
  strcpy(uri_dir, uri);
  ptr = strrchr(uri_dir, '/');
  *(++ptr) = '\0';
  return uri_dir;
...
void StoreEntrypurgedir(hash_table *hid, const cache_key *key, int *t) {
  StoreEntry *e = NULL;
  hash_link *walker;
  int i;
  for (i = 0; i < hid->size; i++)
  {
    for (walker = hid->buckets[i]; walker != NULL; walker = walker->next)
    {
       e = (StoreEntry *)walker;
      if (NULL == e->dirkey)
        continue;
      if ((hid->cmp)(key, e->dirkey) == 0)
      {
      	 storeRelease(e);
        (*t)++;
      }
...
storeCreateEntry(const char *url, request_flags flags, method_t method)
{
    ...
    if(flags.cachable)
      e->dirkey = storeKeyDup(storeKeyPublic(fetchuri_dir(url), method));
    return e;
}
