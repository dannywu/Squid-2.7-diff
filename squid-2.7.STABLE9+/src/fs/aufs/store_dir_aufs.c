...
static StoreEntry *storeAufsDirAddDiskRestore(SwapDir * SD, const cache_key * key, const cache_key * dirkey,
    sfileno file_number,
...
storeAufsDirRebuildFromDirectory(void *data)
{
    ...
    cache_key dirkey[SQUID_MD5_DIGEST_LENGTH];
    sfileno filn = 0;
    int count;
    int size;
    struct stat sb;
	...
	e = storeAufsDirAddDiskRestore(SD, key, dirkey,
	    filn,
	    tmpe.swap_file_sz,
	    tmpe.expires,
	    tmpe.timestamp,
...
storeAufsDirRebuildFromSwapLog(void *data)
{
	...
	rb->counts.objcount++;
	e = storeAufsDirAddDiskRestore(SD, s.key, s.dirkey,
	    s.swap_filen,
	    s.swap_file_sz,
	    s.expires,
	    s.timestamp,
...
static StoreEntry *
storeAufsDirAddDiskRestore(SwapDir * SD, const cache_key * key, const cache_key * dirkey,
    sfileno file_number,
    squid_file_sz swap_file_sz,
    time_t expires,
    time_t timestamp,
