...
static StoreEntry *storeUfsDirAddDiskRestore(SwapDir * SD, const cache_key * key, const cache_key *dirkey,
    int file_number,
...
storeUfsDirRebuildFromDirectory(void *data)
{
    ...
    cache_key dirkey[SQUID_MD5_DIGEST_LENGTH];//----add by wuhongxing 
    sfileno filn = 0;
    int count;
    int size;
    struct stat sb;
	...
	memset(key, '\0', SQUID_MD5_DIGEST_LENGTH);
        memset(dirkey, '\0', SQUID_MD5_DIGEST_LENGTH);
	...
	for (t = tlv_list; t; t = t->next) {
		...
		break;
            //-----------add by wuhongxing 
            case STORE_META_KEY_URL:
                assert(t->length == SQUID_MD5_DIGEST_LENGTH);
                xmemcpy(dirkey, t->value, SQUID_MD5_DIGEST_LENGTH);
                break;
           //-----------------
	...
	e = storeUfsDirAddDiskRestore(SD, key, dirkey,
	    filn,
	    tmpe.swap_file_sz,
	    tmpe.expires,
	    tmpe.timestamp,
...
storeUfsDirRebuildFromSwapLog(void *data)
{
	...
	rb->counts.objcount++;
	e = storeUfsDirAddDiskRestore(SD, s.key, s.dirkey,
	    s.swap_filen,
	    s.swap_file_sz,
	    s.expires,
	    s.timestamp,
...
static StoreEntry *
storeUfsDirAddDiskRestore(SwapDir * SD, const cache_key * key, const cache_key *dirkey,
    int file_number,
    squid_file_sz swap_file_sz,
    time_t expires,
    time_t timestamp,
    ...
    int clean)
{
    ...
    e->refcount = refcount;
    e->flags = flags;
    e->dirkey = storeKeyDup(dirkey);// add by wuhongxing
...
storeUfsDirWriteCleanEntry(SwapDir * sd, const StoreEntry * e)
{
    ...
    xmemcpy(&s.dirkey, e->dirkey, SQUID_MD5_DIGEST_LENGTH);//-------------add by wuhongxing 
...
//-----add by wuhongxing 20110720
    xmemcpy(s->dirkey, e->dirkey, SQUID_MD5_DIGEST_LENGTH);
//-----------------
