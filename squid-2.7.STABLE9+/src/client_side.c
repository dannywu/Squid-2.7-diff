...
//--------------------------------------------------------add by dannywu at 20120719

static StoreEntry *cache_get_CreateStoreEntry(clientHttpRequest * h, method_t, request_flags);
static void clientGetCacheobjRequest(clientHttpRequest * );
static void clientGetCacheobjRelease(clientHttpRequest * );
static int Purgedirsotre(const char *uri_dir, const method_t method);
static int checkuri_dir(const char *uri);
static void clientGetCacheobjReRange(clientHttpRequest *);

static StoreEntry *cache_get_CreateStoreEntry(clientHttpRequest * h, method_t m, request_flags flags)
{
    StoreEntry *e;
     /*
...
static void clientGetCacheobjRequest(clientHttpRequest * http)
{
    char *url = http->uri;
	...
	http->entry = cache_get_CreateStoreEntry(http, r->method, null_request_flags);
    ...
    http->entry = cache_get_CreateStoreEntry(http, r->method, r->flags);
    ...
    httpReplyReset(re = http->entry->mem_obj->reply);
    httpBuildVersion(&version, 1, 0);
    httpReplySetHeaders(re, status, NULL, NULL, 0, 0, -1);
    httpReplySwapOut(re, http->entry);
    storeComplete(http->entry);     
}

static void clientGetCacheobjRelease(clientHttpRequest * http)
{
    http_status status = HTTP_NOT_MODIFIED;
    http_version_t version;
    HttpReply *re = NULL;
    http->entry = clientCreateStoreEntry(http, http->request->method, null_request_flags);
    httpReplyReset(re = http->entry->mem_obj->reply);
    httpBuildVersion(&version, 1, 0);
    httpReplySetHeaders(re, status, "Object has been", NULL, 0, 0, -1);
    httpReplySwapOut(re, http->entry);
    storeComplete(http->entry);	
}
...
static int Purgedirsotre(const char *uri_dir, const method_t method)
{
  //cache_key *dirkey = MD5String(uri_dir);
  int tmp = 0;
  const cache_key *dirkey = storeKeyPublic(uri_dir, method);
  StoreEntrypurgedir(store_table, dirkey, &tmp);
  return tmp;
}
...
static void clientGetCacheobjReRange(clientHttpRequest * http)
{
  clientHttpRequest *newhttp;
  char *url = http->uri;;
  //HttpReply *re;
  newhttp = cbdataAlloc(clientHttpRequest);
  newhttp->conn = http->conn;
  newhttp->start = current_time;
  newhttp->req_sz = strlen(url);
  newhttp->uri = xstrdup(url);
  newhttp->range_iter.boundary = StringNull;
  httpBuildVersion(&newhttp->http_ver, 1, 0);
  dlinkAdd(newhttp, &newhttp->active, &ClientActiveRequests);
  request_t *r = newhttp->request = requestLink(requestCreate(http->request->method, http->request->protocol, strBuf(http->request->urlpath)));
  strcpy(r->host, http->request->host);
  r->http_ver = http->request->http_ver;
  r->port = http->request->port;
  memcpy(&r->flags, &http->request->flags, sizeof(request_flags));
  StoreEntry *e = newhttp->entry = storeCreateEntry(url, http->request->flags, http->request->method);
  if (Config.onoff.collapsed_forwarding && r->flags.cachable && !r->flags.need_validation && (r->method == METHOD_GET || r->method == METHOD_HEAD)) {
    e->mem_obj->refresh_timestamp = squid_curtime;
  /* Set the vary object state */
    safe_free(e->mem_obj->vary_headers);
    if (r->vary_headers)
          e->mem_obj->vary_headers = xstrdup(r->vary_headers);
    safe_free(e->mem_obj->vary_encoding);
    if (strBuf(r->vary_encoding))
          e->mem_obj->vary_encoding = xstrdup(strBuf(r->vary_encoding));
    e->mem_obj->request = requestLink(r);
    EBIT_SET(e->flags, KEY_EARLY_PUBLIC);
    storeSetPublicKey(e);
  }
  //fwdStart(newhttp->conn->fd, e, r);
  fwdStart(newhttp->conn->fd, e, r);
}
...
//-----add by wuhongxing 20110720
    if (checkuri_dir(http->uri)){
      if ( Purgedirsotre(http->uri, METHOD_GET) != 0 )
         status = HTTP_OK;
      if ( Purgedirsotre(http->uri, METHOD_HEAD) != 0 )
      status = HTTP_OK;
    }
//-----------------------
    /*
...
//------------------------------------------------------------------------------
    if ( r->method == METHOD_COPY){
	r->method = METHOD_GET;
	if( storeGetPublic(http->uri, METHOD_GET) == NULL )
	    clientGetCacheobjRequest(http);
	else
	    clientGetCacheobjRelease(http);
	return;
    }
    if ((httpHeaderFindEntry(&r->header, HDR_RANGE)) ||
        (httpHeaderFindEntry(&r->header, HDR_REQUEST_RANGE))) {
        clientGetCacheobjReRange(http);
    }
//-------------------------------------------------------------modify by dannywu at 20120719
    if (clientOnlyIfCached(http)) {
	clientProcessOnlyIfCachedMiss(http);
	return;
    }
